...
class Score(Plugin):
    ...
    def calculate(self, nzb, movie):
        ...
        if nzb.get('seeders'):
            try:
                score += min(nzb.get('seeders') * 100 / 15, Env.setting('max_seeders_score', section = 'searcher'))
                score += min(nzb.get('leechers') * 100/ 30, Env.setting('max_leechers_score', section = 'searcher'))
            except:
                pass
